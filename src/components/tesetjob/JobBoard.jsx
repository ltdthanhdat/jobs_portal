...
export default function JobBoard({ title, props }) {
	...
	const [placeholderText, setPlaceholderText] = useState(" Nhấn (/) để tìm kiếm");
			...
			<div className="bg-light">
				<div className="md:w-10/12 w-11/12 mx-auto h-full md:pb-28 pb-12">
					{title === false ? (
						<div className="block pt-4">
							<h1 className="md:text-6xl text-4xl font-bold text-gray-900 text-center md:pb-16 pb-12 pt-10">
								Công việc nổi bật ({limitedJobs.length})
							</h1>
						</div>
					) : (
						<div className="mb-10">
                    ...
                    hover:border-blue-500"
										...
										onFocus={() => setPlaceholderText("Tìm kiếm tên công việc")}
										onBlur={() => setPlaceholderText("Nhấn (/) để tìm kiếm")}
										ref={searchRef}
									/>
								</div>
							</div>
					...
					{title === false ? (
						<div className="grid lg:grid-cols-3 gap-6 grid-cols-1 mx-2 ">
							{limitedJobs.length > 0 ? (
								...
								<h5 style={{ textAlign: "center" }}>Không tìm thấy công việc nào</h5>
							)}
						</div>
					) : (
