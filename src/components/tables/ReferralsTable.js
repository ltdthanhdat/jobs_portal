		...
		<>
											...
											return (
												<React.Fragment key={index}>
													<tr>
														...
														<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
															<div>
																Ngày ứng tuyển: {appliedOn.toLocaleDateString()}
															</div>
															{obj.status === "accepted" ||
																obj.status === "finished" ? (
																<div>
																	Ngày tham gia: {joinedOn.toLocaleDateString()}
																</div>
															) : (
																<div>
			...
			<div>
				<span className="font-normal text-slate-500">
					<span className="text-red-500">*</span>Nếu công việc được{" "}
					<span className="font-semibold uppercase">chấp nhận</span>, nhấn vào
					<span className="font-semibold uppercase">'Đánh giá công việc'</span> để
					đánh giá.
					...
					<div className="min-h-screen px-4 text-center">
						...
						>
							<div className="inline-block w-full max-w-lg md:p-6 p-3 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
								<h1 className="text-center text-3xl font-semibold">Đánh giá</h1>
